...
from.rfdetr_segmenter import RFDETRSegmenter
...
class DVIS_online(MinVIS):
    ...
    def reset_image_output_order(self, output, indices):
        ...
        frame_indices = torch.arange(0, indices.shape[0]).to(indices).unsqueeze(1).repeat(1, indices.shape[1])
...
# ===========================================================================
#  RF-DETR backed DVIS variants
# ===========================================================================

@META_ARCH_REGISTRY.register()
class DVIS_online_rfdetr(nn.Module):
    """
    Online version of DVIS using RF-DETR as the per-frame segmenter
    instead of Mask2Former.
    """
    ...
    @classmethod
    def from_config(cls, cfg):
        segmenter = RFDETRSegmenter(cfg)

        # Loss parameters:
        deep_supervision = cfg.MODEL.MASK_FORMER.DEEP_SUPERVISION
        no_object_weight = cfg.MODEL.MASK_FORMER.NO_OBJECT_WEIGHT

        # loss weights (keep DVIS's own dice + ce + mask bce losses)
        class_weight = cfg.MODEL.MASK_FORMER.CLASS_WEIGHT
        dice_weight = cfg.MODEL.MASK_FORMER.DICE_WEIGHT
        mask_weight = cfg.MODEL.MASK_FORMER.MASK_WEIGHT
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            self.segmenter.eval()
            with torch.no_grad():
                image_outputs = self.segmenter(images.tensor)
                frame_embds = image_outputs['pred_embds'].clone().detach()  # (b, c, t, q)
                mask_features = image_outputs['mask_features'].clone().detach().unsqueeze(0)
                del image_outputs['mask_features']
                torch.cuda.empty_cache()
    ...
    def run_window_inference(self, images_tensor, window_size=30):
        ...
        for i in range(iters):
            ...
            end_idx = (i + 1) * window_size
            # segmenter inference
            out = self.segmenter(images_tensor[start_idx:end_idx])
            # referring tracker inference
            frame_embds = out['pred_embds']  # (b, c, t, q)
            mask_features = out['mask_features'].unsqueeze(0)
            # remove unnecessary variables to save GPU memory
    ...
    def inference_video_vis(
        ...
        if len(pred_cls) > 0:
            ...
            if aux_pred_cls is not None:
                ...
                scores = torch.maximum(scores, aux_pred_cls.to(scores))
            labels = torch.arange(
                self.segmenter.num_classes, device=self.device
            ).unsqueeze(0).repeat(self.num_queries, 1).flatten(0, 1)
            scores_per_image, topk_indices = scores.flatten(0, 1).topk(self.max_num, sorted=False)
            labels_per_image = labels[topk_indices]
            topk_indices = topk_indices // self.segmenter.num_classes
            pred_masks = pred_masks[topk_indices]
            pred_ids = pred_id[topk_indices]
    ...
    def inference_video_vps(
        ...
        keep = labels.ne(self.segmenter.num_classes) & (scores > self.object_mask_threshold)
        cur_scores = scores[keep]
        cur_classes = labels[keep]
        cur_ids = pred_id[keep]
        cur_masks = mask_pred[keep]
